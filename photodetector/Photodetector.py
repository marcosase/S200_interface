...
# from phiola.data.datafiles import PhiDataLoader
...
class KeithleyPD(Photodetector):
    ...
    def __init__(self):
        super().__init__()
        super().set_ID(ID='KEITHLEY 2520-INT')
...
#    p.set_ID(ID='THORLABS S148C')
    p.set_ID(ID='KEITHLEY 2520-INT')

    p.set_wavelength(wvl=1310)
    r=p.get_responsivity()
    print (r)
    p=KeithleyPD()
    #p.set_wavelength(wvl=1551)
    r=p.get_responsivity(wvl=1310)
