...
from load_sample_V2  import LoadSample
...
class Ui(QtWidgets.QMainWindow):
	def __init__(self):
		...
		##
		self.jobs_combo_box1.currentTextChanged.connect(self.update_info1)
		self.jobs_combo_box2.currentTextChanged.connect(self.update_info2)
		self.jobs_combo_box3.currentTextChanged.connect(self.update_info3)
		self.jobs_combo_box4.currentTextChanged.connect(self.update_info4)
		self.show() # Show the GUI
		self.Wafer1.setValue(0)
	...
	def write_values(self):
		try:
			...
			#set th values
			self.table_wfr1.setItem(0, 0, QTableWidgetItem(str( int(df.Th1[0]) )))
			self.table_wfr2.setItem(0, 0, QTableWidgetItem(str( int(df.Th2[0]) )))
			self.table_wfr3.setItem(0, 0, QTableWidgetItem(str( int(df.Th3[0]) )))
			self.table_wfr4.setItem(0, 0, QTableWidgetItem(str( int(df.Th4[0]) )))
			# set xy touch down values
			self.table_wfr1.setItem(0, 1, QTableWidgetItem(str( int(df.X1[0]) )))
			self.table_wfr1.setItem(0, 2, QTableWidgetItem(str( int(df.Y1[0]) )))
			self.table_wfr2.setItem(0, 1, QTableWidgetItem(str( int(df.X2[0]) )))
			self.table_wfr2.setItem(0, 2, QTableWidgetItem(str( int(df.Y2[0]) )))
			self.table_wfr3.setItem(0, 1, QTableWidgetItem(str( int(df.X3[0]) )))
			self.table_wfr3.setItem(0, 2, QTableWidgetItem(str( int(df.Y3[0]) )))
			self.table_wfr4.setItem(0, 1, QTableWidgetItem(str( int(df.X4[0]) )))
			self.table_wfr4.setItem(0, 2, QTableWidgetItem(str( int(df.Y4[0]) )))
			#Set job files
			self.update_info1(str(df.job1[0]))
			self.update_info2(str(df.job2[0]))
			self.update_info3(str(df.job3[0]))
			self.update_info4(str(df.job4[0]))
	...
	def file_save(self, val):
		dic = {'Th1':[val[0]],'Th2':[val[1]],'Th3':[val[2]],'Th4':[val[3]],
		'X1':[val[4]],'Y1':[val[8]],
		'X2':[val[5]],'Y2':[val[9]],
		'X3':[val[6]],'Y3':[val[10]],
		'X4':[val[7]],'Y4':[val[11]],
	...
	def collect(self):
		...
		x_td_arr      = [self.table_wfr1.item(0, 1).text(),
					...
					self.table_wfr4.item(0, 1).text()]
		y_td_arr      = [self.table_wfr1.item(0, 2).text(),
					self.table_wfr2.item(0, 2).text(),
	...
	def touch_bar2(self):
		...
		self.start_jobs_btn.repaint()
		self.send_command(b'GUP\n') # Do a Gross up
		theta = self.table_wfr2.item(0, 0).text()
		...
		self.table_wfr2.setItem(0, 1, QTableWidgetItem(str(x2)))
		self.table_wfr2.setItem(0, 2, QTableWidgetItem(str(y2)))
		#save values
		self.file_save(self.collect())
		self.reset_start_btn()
	def touch_bar3(self):
		...
		self.start_jobs_btn.repaint()
		self.send_command(b'GUP\n') # Do a Gross up
		theta = self.table_wfr3.item(0, 0).text()
		command = f'GTTH {theta}\n'.encode()
		self.send_command(command)
		self.send_command(b'LDPH 0\n') #Go to local and the asks the user to perform a touchdown
		self.wait_ser()
		...
		self.table_wfr3.setItem(0, 1, QTableWidgetItem(str(x3)))
		self.table_wfr3.setItem(0, 2, QTableWidgetItem(str(y3)))
		#save values
		self.file_save(self.collect())
		self.reset_start_btn()
	def touch_bar4(self):
		...
		self.start_jobs_btn.repaint()
		self.send_command(b'GUP\n') # Do a Gross up
		theta = self.table_wfr4.item(0, 0).text()
		command = f'GTTH {theta}\n'.encode()
		self.send_command(command)
		self.send_command(b'LDPH 0\n') #Go to local and the asks the user to perform a touchdown
		self.wait_ser()
		...
		self.table_wfr4.setItem(0, 1, QTableWidgetItem(str(x4)))
		self.table_wfr4.setItem(0, 2, QTableWidgetItem(str(y4)))
		#save values
		self.file_save(self.collect())
		self.reset_start_btn()
	def update_combo_box(self,combo_object, array):
		combo_object.addItems(array)
	def select_job1(self):
	...
	def update_info1(self, path1=''):
		...
		if path1 is not '':
			...
			job_selected = self.jobs_combo_box1.currentText()
		...
		#Update the labels
		self.cust_id1.setText(cust_id)
		self.batch_id1.setText(batch_id)
		self.wafer_id1.setText(wafer_id)
		self.prod_id1.setText(prod_id)
		self.job_id1.setText(filename)
		#getting the MMF
	...
	def go_td1(self):
		...
		command = f'GTTH {theta}\n'.encode()
		self.send_command(command)
		xtd = self.table_wfr1.item(0, 1).text()
		ytd = self.table_wfr1.item(0, 2).text()
	...
	def go_td2(self):
		...
		command = f'GTTH {theta}\n'.encode()
		self.send_command(command)
		xtd = self.table_wfr2.item(0, 1).text()
		ytd = self.table_wfr2.item(0, 2).text()
		command = f'GTXY {xtd},{ytd}\n'.encode()
		self.send_command(command)
	def go_td3(self):
		self.send_command(b'LI1\n')
		...
		command = f'GTTH {theta}\n'.encode()
		self.send_command(command)
		xtd = self.table_wfr3.item(0, 1).text()
		ytd = self.table_wfr3.item(0, 2).text()
		command = f'GTXY {xtd},{ytd}\n'.encode()
		self.send_command(command)
	def go_td4(self):
		self.send_command(b'LI1\n')
		...
		command = f'GTTH {theta}\n'.encode()
		self.send_command(command)
		xtd = self.table_wfr4.item(0, 1).text()
		ytd = self.table_wfr4.item(0, 2).text()
		command = f'GTXY {xtd},{ytd}\n'.encode()
		self.send_command(command)	
	def start_jobs(self):
		...
		stat_wafer1 = self.check_wafer1.isChecked()
		stat_wafer2 = self.check_wafer2.isChecked()
		stat_wafer3 = self.check_wafer3.isChecked()
		stat_wafer4 = self.check_wafer4.isChecked()
		wfr1 = self.check_wafer1.text()
		wfr2 = self.check_wafer2.text()
		wfr3 = self.check_wafer3.text()
		wfr4 = self.check_wafer4.text()
		prog_arr = ['Wafer1','Wafer2','Wafer3','Wafer4',]
		stat_arr = [stat_wafer1, stat_wafer2, stat_wafer3, stat_wafer4]
		core_path = 'C:/Users/S300/OneDrive - Smart Photonics/Test  Measurement - Engineering/Software/S300_interface/utils/meas_core.py'
		user_name_arr  = [self.op_id1.currentText(),
						self.op_id2.currentText(),
						self.op_id3.currentText(),
						self.op_id4.currentText()]
		job_file_arr   = [self.job_id1.text(), 
						self.job_id2.text(), 
						self.job_id3.text(), 
						self.job_id4.text()]
		...
		x_td_arr      = [int(self.table_wfr1.item(0, 1).text()),
						int(self.table_wfr2.item(0, 1).text()),
						int(self.table_wfr3.item(0, 1).text()),
						int(self.table_wfr4.item(0, 1).text())]
		y_td_arr      = [int(self.table_wfr1.item(0, 2).text()),
						int(self.table_wfr2.item(0, 2).text()),
						int(self.table_wfr3.item(0, 2).text()),
						int(self.table_wfr4.item(0, 2).text())]
		if stat_arr==[False,False,False,False]:
				self.start_jobs_btn.setStyleSheet('background-color: rgb(255, 255, 0)')
				self.start_jobs_btn.setText("Check a wafer to measure")
				self.start_jobs_btn.repaint()
				time.sleep(1)
		for i, j, k, l, m, p, prog in  zip(job_file_arr, user_name_arr, theta_arr, x_td_arr, y_td_arr, stat_arr, prog_arr):
			...
			if p:
				...
				self.start_jobs_btn.repaint()
				#Add status message
				#call(['python', core_path, str(i),str(j),str(k),str(l),str(m)])#i: job_arr, j:user, k:theta, l: x_td, m: y_td
				#call(['python',teste_path])
				self.run(i, j, k, l, m, prog)
    ...
    def read_job_v2(self): # ABI: this function should replace read_job()
        """ Reads the JOB file and sets the general acquisition settings
        """

        print('\nPlease select the JOB file via the file dialog.\n')
        self.fe.get_job_fullpath()
        self.set_job_folder_fullpath(fp=self.fe.job)
        self.fe.destroy()#ABI: temporary hack, to be removed asap
    ...
    def run(self):
        ...
        """
        self.init_process()
        #self.init_probemonitor('COM10') #TODO: move this device and port to the EDF #Commented by marcos  microcontroler is dead
    ...
    def init_process(self):
        ...
        self.generate_meas_plan()# ABI: new function
    ...
    def match_coordinates(self):
        ...
        """
        self.mp.match_cell_positions_to_probing_stage(x0=self.alignment.x0, y0=self.alignment.y0)
    ...
    def perform_measurement_loop(self): # ABI: this function is aimed at replacing init_measurement_loop()
        ...
        if self.meas_procedure == 'die_wise': # all measurements on one die in one go
            for cell in tqdm(self.mp.get_cell_names, bar_format='{l_bar}%s{bar:75}%s{r_bar}' % (Fore.YELLOW, Fore.RESET),desc='Measurement Progress'):
        ...
        if self.meas_procedure == 'meas_wise': # each measurement on all dies, before passing to next measurement
            ...
            #for ms in tqdm(self.mp.planned_meas, bar_format='{l_bar}%s{bar:75}%s{r_bar}' % (Fore.YELLOW, Fore.RESET),desc='Measurement Progress'):
                ...
                for cell in tqdm(self.mp.get_cells_for_meas_plan(meas_name=ms), bar_format='{l_bar}%s{bar:75}%s{r_bar}' % (Fore.YELLOW, Fore.RESET),desc='Measurement Progress'):
                    self.go_to_cell_v2(cl=cell)
                    if ms == 'PICTURE':
                        self.take_picture(picture_name='_'.join([self.wafer, self.batch, cell, ms])) 
                    else:
    ...
    def identify(self):
        ...
        '''
        self.operator         =    self.idntf.operator()
    def init_equipment(self):
        ...
        #except:
        #from emulators import DummyEquipment
        #self.eq=DummyEquipment(fullpath=self.edf_fullpath)ds
        #print ('\n\n**********EMULATION MODE**********\n\n')
