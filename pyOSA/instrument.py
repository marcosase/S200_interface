...
# Keep track of initialized devices
_initialized_devices = defaultdict(lambda: False)
...
class Instrument:
    ...
    def __init__(self, spectrometer_index: int, virtual:bool=False):
        ...
        if _initialized_devices[self.serial_number]:
            raise RuntimeError(f"Error {self.model} ({self.serial_number}) has already been initialized!")
        _initialized_devices[self.serial_number] = True
    ...
    def close(self):
        ...
        try:
            ...
            _initialized_devices[self.serial_number] = False
            if status != 0:
                raise RuntimeError(
        ...
        logger.info(f"OSA connection to spectrometer {self.model} ({self.serial_number}) "
    ...
    def _check_spectrometer(spectrometer_index:int, ignore_errors: typing.List[str] = []):
        ...
        if status == 266:
            ...
            logger.error(msg)
            # Lets make all the strings lowercase
            ignore_errors = [error.lower() for error in ignore_errors]
            if not "Usb Transfer Error".lower() in ignore_errors:
                # Usb Transfer Error is handled in FTSLib, so we
                # can ignore it here
                ignore_errors.append("Usb Transfer Error".lower())
            if all(error.lower() in ignore_errors for error in instrumentstatus):
    ...
    def set_apodization(self, apodization: str) -> None:
        ...
        """
        apodization_type_nr = constants.convert_apodization_string_to_number(apodization)

        status = FTSLib.FTS_SetAcquisitionOption_ApodizationType(
            self.spectrometer_index, apodization_type_nr
        )
        if status != 0:
            raise AcquisitionException(
    ...
    def _clear_data(self):
        ...
        This can be used after reading the data in python to free up some space
        """
    ...
    def __wait_for_queue(self, q, msg, nr):
        ...
        while q.qsize() < nr:
            ...
            if time.time() - lastmsg > 1:
                ...
                if not _initialized_devices[self.serial_number]:
                    raise IOError(f"Connection to {self.model} ({self.serial_number}) is not open.")
    ...
    def _get_last_spectrum_channel(self, channel):
        ...
        spec = spectrum_t(allocated_length_I=length.value, allocated_length_x=length.value)
    ...
    def _get_last_interferogram_channel(self, channel_index):
        ...
        interferogram = spectrum_t(allocated_length_I=length.value)
