...
def guess_dll_path():
    ...
    """
    envvariable = os.getenv('FTSLIB_PATH', None)
    if envvariable is not None:
        return envvariable
...
def load_dll(dll_path: str=None):
    ...
    try:
        os.environ["PATH"] = os.path.dirname(dll_path) + os.pathsep + os.environ["PATH"] # Add the DLL folder to path
    except Exception as e:
        print(f"Couldn't add {os.path.dirname(dll_path)} to path: {e}")
    cdll = c.CDLL(dll_path) # Load the dll
    check_version(cdll)
    return cdll


FTSLib = load_dll('C:\Program Files\Thorlabs\ThorSpectra\FTSLib.dll')
